-- Behavior Savior Analytics Database Schema - SAFE VERSION
...
-- Add additional columns for intervention tracking
...
-- Update existing hexie cards with behavior savior data (safe update)
...
CREATE INDEX IF NOT EXISTS idx_user_interactions_hexie_card ON user_interactions(hexie_card_id);
CREATE INDEX IF NOT EXISTS idx_user_interactions_user_time ON user_interactions(user_id, created_at);

-- Row Level Security for analytics data
ALTER TABLE user_interactions ENABLE ROW LEVEL SECURITY;

-- Drop existing policy if it exists
DROP POLICY IF EXISTS analytics_access_policy ON user_interactions;

-- Create RLS policy for user interactions
...
DO $$
BEGIN
  RAISE NOTICE 'Behavior Savior analytics schema installed successfully!';
  RAISE NOTICE 'No sample data was inserted - you can add real interaction data as users interact with hexies.';
END $$;