-- Behavior Savior Analytics Database Schema
...
-- Anonymized behavior patterns view for analytics
...
SELECT 
  ...
  hc.subcategory,
  AVG(COALESCE(ui.severity_rating, hc.severity_rating)) as avg_severity,
  COUNT(*) as pattern_frequency,
  hc.psychological_framework as framework,
  hc.environmental_factors,
  FLOOR(AVG(COALESCE(ui.severity_rating, hc.severity_rating)) * 10) as impact_score,
  array_agg(DISTINCT extract(week from ui.created_at)) as trend_data,
...
BEGIN
  ...
  SELECT COUNT(*), AVG(avg_severity), 
         (SELECT AVG(avg_severity) FROM behavior_patterns_anonymous WHERE last_updated >= NOW() - INTERVAL '30 days') -
         (SELECT AVG(avg_severity) FROM behavior_patterns_anonymous WHERE last_updated >= NOW() - INTERVAL '60 days' AND last_updated < NOW() - INTERVAL '30 days')
...
BEGIN
  ...
  SELECT 
    ...
    AVG(hc.severity_rating)::NUMERIC as avg_severity,
    ...
    (5 - AVG(hc.severity_rating))::NUMERIC as intervention_score,
