...
interface HoneycombHexieMenuProps {
  ...
  onDragStart?: (hexie: HexieCard) => void;
  onDragEnd?: () => void;
}
...
export const HoneycombHexieMenu: React.FC<HoneycombHexieMenuProps> = ({
  hexieCards,
  userTier,
  favorites,
  onHexieSelect,
  ...
  const [hoveredHexies, setHoveredHexies] = useState<Set<string>>(new Set());
  const [hexPositions, setHexPositions] = useState<Map<string, HexPosition>>(new Map());
  ...
  const filteredHexies = React.useMemo(() => {
    ...
    // Filter to show only free tier hexies (as requested)
  ...
  const calculateHoneycombPositions = useCallback(() => {
    ...
    filteredHexies.forEach((hexie, index) => {
      ...
      positions.set(hexie.id, { x, y, row, col });
    });
    
    setHexPositions(positions);
  }, [filteredHexies, HEX_SIZE, HEX_SPACING]);
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        // Fallback to extracting from hexie cards
        const fallbackCats = [...new Set(hexieCards.map(h => h.category))].filter(Boolean);
  ...
  const handleDragStart = (e: React.DragEvent, hexie: HexieCard) => {
    e.dataTransfer.setData('text/plain', JSON.stringify(hexie));
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (!container || zoomedHexie) return; // Don't scroll when zoomed
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!scrollContainerRef.current || filteredHexies.length === 0) return;
      ...
      switch (e.key) {
        ...
        case ' ':
          if (zoomedHexie) {
            e.preventDefault();
            const hexie = filteredHexies.find(h => h.id === zoomedHexie);
            if (hexie) {
              onHexieSelect(hexie);
            }
          }
  ...
  }, [filteredHexies, HEX_SIZE, HEX_SPACING, zoomedHexie]);
  ...
  return (
    <Card className="w-[28rem] h-full bg-gray-900/95 border-gray-700 flex flex-col z-30" data-tessera-library>
      ...
      <div 
        ...
        tabIndex={0}
        role="grid"
        aria-label="Hexies library grid"
        style={{
          filter: zoomedHexie ? 'blur(1px)' : 'none',
      ...
      >
        ...
        <div 
          ...
          style={{ 
            height: Math.max(containerHeight, filteredHexies.length > 0 ? 
              Math.ceil(filteredHexies.length / HEXES_PER_ROW) * (HEX_SIZE * Math.sqrt(3) / 2 + HEX_SPACING) * 0.75 + 100 
              : 200),
            width: Math.max(800, HEXES_PER_ROW * (HEX_SIZE + HEX_SPACING) + 200), // Ensure horizontal scrolling
            filter: zoomedHexie ? 'blur(0.5px)' : 'none',
            transition: 'filter 0.3s ease'
          }}
        >
          {filteredHexies.length === 0 ? (
            ...
            filteredHexies.map((hexie) => {
              const position = hexPositions.get(hexie.id);
              if (!position) return null;

              const isZoomed = zoomedHexie === hexie.id;
              const isFavorite = favorites.includes(hexie.id);
              const isFlipped = flippedHexies.has(hexie.id);
              const isHovered = hoveredHexies.has(hexie.id);
              ...
              return (
                <div
                  ...
                  onDragStart={(e) => handleDragStart(e, hexie)}
                  onDragEnd={handleDragEnd}
                  onMouseEnter={() => setHoveredHexies(prev => new Set([...prev, hexie.id]))}
                  onMouseLeave={() => setHoveredHexies(prev => {
                    const newSet = new Set(prev);
                    newSet.delete(hexie.id);
                    return newSet;
                  })}
                  onClick={(e) => {
                    e.stopPropagation();
                ...
                >
                  <div className="relative">
                    <HexagonShape
                      size={HEX_SIZE}
                      color={hexie.color_scheme?.primary || '#3b82f6'}
                      borderColor={hexie.color_scheme?.secondary || '#1e40af'}
                      shadowColor="none"
                      title={hexie.title}
                      frontText={hexie.front_text}
                      backText={hexie.back_text}
                      references={hexie.references || hexie.card_references || []}
                      showFlipButton={false}
                      isFlipped={isFlipped}
                      onFlip={() => handleFlipHexie(hexie.id)}
                        ...
                        <div className="flex space-x-1 bg-black/70 backdrop-blur-sm rounded-lg p-1.5">
                          <Button
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              handleToggleFavorite(hexie.id, e);
                            }}
                            size="sm"
                            variant="ghost"
                    ...
                    {hexie.subscription_tier_required !== 'free' && !isZoomed && !isHovered && (
                      <div className="absolute -top-0.5 -left-0.5">
                        <div className={`h-3 w-3 rounded-full ${
                          hexie.subscription_tier_required === 'premium' 
                            ? 'bg-yellow-400' 
                            : 'bg-blue-400'
      ...
      {/* Zoomed Hexie Overlay - Fixed Position with Background Blur */}
      {zoomedHexie && (() => {
        const hexie = filteredHexies.find(h => h.id === zoomedHexie);
      ...
      <div className="p-3 border-t border-gray-700 bg-gray-800/50">
        <div className="text-xs text-gray-400 space-y-1">
          <p>• Hover hexies: Quick actions (♥ Favorite, + Add to Workspace, 🔍 Zoom)</p>
          <p>• Click hexies: Zoom to read full text and flip front/back</p>
          <p>• Auto-scroll: Move mouse near edges (↑↓←→) • Drag hexies to workspace</p>
        </div>
      </div>
    </Card>
  );
...
export default HoneycombHexieMenu;