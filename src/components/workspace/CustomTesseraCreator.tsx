...
interface CustomTesseraCreatorProps {
  userTier: 'free' | 'basic' | 'premium';
  onTesseraCreated: (tessera: TesseraCard) => void;
  onClose: () => void;
  isOpen: boolean;
}

interface CustomTesseraData {
  title: string;
  frontText: string;
  backText: string;
  category: string;
...
export const CustomTesseraCreator: React.FC<CustomTesseraCreatorProps> = ({
  userTier,
  onTesseraCreated,
  onClose,
  isOpen
}) => {
  const [tesseraData, setTesseraData] = useState<CustomTesseraData>({
    title: '',
    frontText: '',
    backText: '',
    category: 'Custom',
  ...
  const validateForm = () => {
    ...
    if (!tesseraData.title.trim()) {
      newErrors.title = 'Title is required';
    } else if (tesseraData.title.length > limits.maxTitleLength) {
      newErrors.title = `Title must be ${limits.maxTitleLength} characters or less`;
    }

    if (!tesseraData.frontText.trim()) {
      newErrors.frontText = 'Front text is required';
    } else if (tesseraData.frontText.length > limits.maxTextLength) {
      newErrors.frontText = `Front text must be ${limits.maxTextLength} characters or less`;
    }

    if (!tesseraData.backText.trim()) {
      newErrors.backText = 'Back text is required';
    } else if (tesseraData.backText.length > limits.maxTextLength) {
  ...
  const handleSave = () => {
    ...
    const newTessera: TesseraCard = {
      id: `custom-${Date.now()}`,
      title: tesseraData.title,
      front_text: tesseraData.frontText,
      back_text: tesseraData.backText,
      category: tesseraData.category,
      tags: tesseraData.tags,
      subscription_tier_required: 'free',
      color_scheme: tesseraData.colorScheme,
    ...
    onTesseraCreated(newTessera);
    
    if (limits.canSavePermanently) {
      toast.success('Custom tessera created and saved!');
    } else {
      toast.success('Custom tessera created temporarily (upgrade to save permanently)');
    }
  ...
  const renderContentStep = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        <input
          type="text"
          value={tesseraData.title}
          onChange={(e) => setTesseraData(prev => ({ ...prev, title: e.target.value }))}
        ...
        <div className="flex justify-between text-xs mt-1">
          <span className={errors.title ? 'text-red-400' : 'text-gray-400'}>
            {errors.title || `${tesseraData.title.length}/${limits.maxTitleLength} characters`}
          </span>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          value={tesseraData.frontText}
          onChange={(e) => setTesseraData(prev => ({ ...prev, frontText: e.target.value }))}
        ...
        <div className="flex justify-between text-xs mt-1">
          <span className={errors.frontText ? 'text-red-400' : 'text-gray-400'}>
            {errors.frontText || `${tesseraData.frontText.length}/${limits.maxTextLength} characters`}
          </span>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          value={tesseraData.backText}
          onChange={(e) => setTesseraData(prev => ({ ...prev, backText: e.target.value }))}
        ...
        <div className="flex justify-between text-xs mt-1">
          <span className={errors.backText ? 'text-red-400' : 'text-gray-400'}>
            {errors.backText || `${tesseraData.backText.length}/${limits.maxTextLength} characters`}
          </span>
        </div>
      </div>
      ...
      <div>
        ...
        <select
          value={tesseraData.category}
          onChange={(e) => setTesseraData(prev => ({ ...prev, category: e.target.value }))}
      ...
      <div>
        ...
        <TagManager
          selectedTags={tesseraData.tags}
          onTagsChange={(tags) => setTesseraData(prev => ({ ...prev, tags }))}
          userTier={userTier}
        />
        <p className="text-xs text-gray-400 mt-1">
          Add tags to help categorize and search for this tessera
        </p>
      </div>
    </div>
  );
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {predefinedColors.map((color, index) => (
            <button
              key={index}
              onClick={() => setTesseraData(prev => ({ ...prev, colorScheme: color }))}
              className={`p-3 rounded-lg border-2 transition-all ${
                tesseraData.colorScheme.primary === color.primary
  ...
  const renderPreviewStep = () => (
    <div className="space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-white mb-4">Preview Your Tessera</h3>
        
        <div className="flex justify-center items-center space-x-4 mb-6">
          <HexagonShape
            size={288}
            title={tesseraData.title || 'Your Title'}
            frontText={tesseraData.frontText || 'Front text will appear here'}
            backText={tesseraData.backText || 'Back text will appear here'}
            color={tesseraData.colorScheme.primary}
            borderColor={tesseraData.colorScheme.secondary}
            isFlipped={isFlipped}
        ...
        <div className="bg-gray-800/50 rounded-lg p-4 text-left">
          <h4 className="font-semibold text-white mb-2">Tessera Details</h4>
          <div className="space-y-2 text-sm">
            <div><span className="text-gray-400">Title:</span> <span className="text-white">{tesseraData.title || 'Not set'}</span></div>
            <div><span className="text-gray-400">Category:</span> <span className="text-white">{tesseraData.category}</span></div>
            <div><span className="text-gray-400">Tags:</span> <span className="text-white">{tesseraData.tags.length > 0 ? tesseraData.tags.join(', ') : 'None'}</span></div>
            <div><span className="text-gray-400">Color:</span> <span className="text-white">{predefinedColors.find(c => c.primary === tesseraData.colorScheme.primary)?.name}</span></div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-xl text-white flex items-center">
                ...
                Create Custom Tessera
              </CardTitle>
              <p className="text-gray-400 text-sm mt-1">
                {userTier === 'free' && 'Free tier: 1 temporary tessera'}
                {userTier === 'basic' && 'Basic tier: 5 permanent tesseras with AI'}
                {userTier === 'premium' && 'Premium tier: 50 permanent tesseras with full features'}
              </p>
            </div>
          ...
          <div className="flex items-center space-x-2 text-sm text-gray-400">
            {!limits.canSavePermanently && (
              <>
                ...
                <span>Temporary tessera (upgrade to save permanently)</span>
              </>
            )}
            {limits.canSavePermanently && (
              <>
...
export default CustomTesseraCreator;